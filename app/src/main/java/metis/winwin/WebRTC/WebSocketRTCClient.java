...
import metis.winwin.WebRTC.RoomParametersFetcher.RoomParametersFetcherEvents;
import metis.winwin.WebRTC.WebSocketChannelClient.WebSocketChannelEvents;
import metis.winwin.WebRTC.WebSocketChannelClient.WebSocketConnectionState;
...
public class WebSocketRTCClient implements AppRTCClient,
  ...
  private void connectToRoomInternal() {
    ...
    //AndLog.ShowLog(TAG, "Connect to room: " + connectionUrl);
    roomState = ConnectionState.NEW;
  ...
  private void disconnectFromRoomInternal() {
    //AndLog.ShowLog(TAG, "Disconnect. Room state: " + roomState);
    if (roomState == ConnectionState.CONNECTED) {
      //AndLog.ShowLog(TAG, "Closing room.");
  ...
  private void signalingParametersReady(
   ...
   // AndLog.ShowLog(TAG, "Room connection completed.");
   ...
   // AndLog.ShowLog(TAG, "Message URL: " + messageUrl);
   // AndLog.ShowLog(TAG, "Leave URL: " + leaveUrl);
  ...
  public void sendAnswerSdp(final SessionDescription sdp) {
    executor.execute(new Runnable() {
      ...
      public void run() {
        if (connectionParameters.loopback) {
         // AndLog.ShowLog(TAG, "Sending answer in loopback mode.");
          return;
        }
        JSONObject json = new JSONObject();
        jsonPut(json, "sdp", sdp.description);
  ...
  public void onWebSocketMessage(final String msg) {
    if (wsClient.getState() != WebSocketConnectionState.REGISTERED) {
      //AndLog.ShowLog(TAG, "Got WebSocket message in non registered state.");
      return;
    }
    try {
  ...
  private void sendPostMessage(
   ...
   // AndLog.ShowLog(TAG, "C->GAE: " + logInfo);
