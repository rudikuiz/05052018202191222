...
public class RoomParametersFetcher {
  ...
  private void roomHttpResponseParse(String response) {
    //AndLog.ShowLog(TAG, "Room response: " + response);
    try {
      ...
      if (!initiator) {
        ...
        for (int i = 0; i < messages.length(); ++i) {
          ...
          //AndLog.ShowLog(TAG, "GAE->C #" + i + " : " + messageString);
          if (messageType.equals("offer")) {
            ...
            //AndLog.ShowLog(TAG, "Unknown message: " + messageString);
          }
        }
      }
      /*AndLog.ShowLog(TAG, "RoomId: " + roomId + ". ClientId: " + clientId);
      AndLog.ShowLog(TAG, "Initiator: " + initiator);
      AndLog.ShowLog(TAG, "WSS url: " + wssUrl);
      AndLog.ShowLog(TAG, "WSS POST url: " + wssPostUrl);
*/
      LinkedList<PeerConnection.IceServer> iceServers =
          iceServersFromPCConfigJSON(roomJson.getString("pc_config"));
      boolean isTurnPresent = false;
      for (PeerConnection.IceServer server : iceServers) {
        //AndLog.ShowLog(TAG, "IceServer: " + server);
      ...
      if (!isTurnPresent) {
        ...
        for (PeerConnection.IceServer turnServer : turnServers) {
          //AndLog.ShowLog(TAG, "TurnServer: " + turnServer);
  ...
  private LinkedList<PeerConnection.IceServer> requestTurnServers(String url)
    ...
    //AndLog.ShowLog(TAG, "Request TURN from: " + url);
    ...
    //AndLog.ShowLog(TAG, "TURN response: " + response);
