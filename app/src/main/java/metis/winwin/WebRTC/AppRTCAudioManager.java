...
import metis.winwin.Utils.AppRTCUtils;
...
public class AppRTCAudioManager {
  ...
  private void onProximitySensorChangedState() {
    ...
    if (audioDevices.size() == 2
      ...
      if (proximitySensor.sensorReportsNearState()) {
        ...
        LocalBroadcastManager.getInstance(apprtcContext).sendBroadcast(new Intent(apprtcContext.getString(R.string.hungustate)).putExtra(apprtcContext.getString(R.string.hungustates), true));
      } else {
        // Sensor reports that a "handset is removed from a person's ear", or
        // "the light sensor is no longer covered".
        //setAudioDevice(AudioDevice.SPEAKER_PHONE);
        LocalBroadcastManager.getInstance(apprtcContext).sendBroadcast(new Intent(apprtcContext.getString(R.string.hungustate)).putExtra(apprtcContext.getString(R.string.hungustates), false));
      }
    }
  }
  ...
  public void setAudioDevice(AudioDevice device) {
    //AndLog.ShowLog(TAG, "setAudioDevice(device=" + device + ")");
    ...
    switch (device) {
      ...
      case WIRED_HEADSET:
        ...
        break;
      default:
        //AndLog.ShowLog(TAG, "Invalid audio device selection");
        break;
    }
  ...
  private void registerForWiredHeadsetIntentBroadcast() {
    ...
    wiredHeadsetReceiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        ...
        /*AndLog.ShowLog(TAG, "BroadcastReceiver.onReceive" + AppRTCUtils.getThreadInfo()
            + ": "
            + "a=" + intent.getAction()
        ...
        switch (state) {
          ...
          case STATE_PLUGGED:
            ...
            break;
          default:
            //AndLog.ShowLog(TAG, "Invalid state");
            break;
        }
      }
    };
  ...
  private void updateAudioDeviceState(boolean hasWiredHeadset) {
    ...
    //AndLog.ShowLog(TAG, "audioDevices: " + audioDevices);
  ...
  private void onAudioManagerChangedState() {
    /*AndLog.ShowLog(TAG, "onAudioManagerChangedState: devices=" + audioDevices
    ...
    if (audioDevices.size() == 2) {
      ...
      //AndLog.ShowLog(TAG, "Invalid device list");
    }
