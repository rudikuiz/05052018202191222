...
import metis.winwin.Utils.AsyncHttpURLConnection;
...
public class WebSocketChannelClient {
  ...
  public void connect(final String wsUrl, final String postUrl) {
    ...
    if (state != WebSocketConnectionState.NEW) {
      //AndLog.ShowLog(TAG, "WebSocket is already connected.");
      return;
    }
    wsServerUrl = wsUrl;
    ...
    //AndLog.ShowLog(TAG, "Connecting WebSocket to: " + wsUrl + ". Post URL: " + postUrl);
  ...
  public void register(final String roomID, final String clientID) {
    ...
    //AndLog.ShowLog(TAG, "Registering WebSocket for room " + roomID + ". CLientID: " + clientID);
    JSONObject json = new JSONObject();
    try {
      json.put("cmd", "register");
      json.put("roomid", roomID);
      json.put("clientid", clientID);
     // AndLog.ShowLog(TAG, "C->WSS: " + json.toString());
  ...
  public void send(String message) {
    ...
    switch (state) {
      ...
      case CONNECTED:
        ...
        // is registered.
       // AndLog.ShowLog(TAG, "WS ACC: " + message);
        wsSendQueue.add(message);
        return;
      case ERROR:
      case CLOSED:
       // AndLog.ShowLog(TAG, "WebSocket send() in error or closed state : " + message);
        return;
      case REGISTERED:
        JSONObject json = new JSONObject();
        try {
         ...
         // AndLog.ShowLog(TAG, "C->WSS: " + message);
  ...
  public void disconnect(boolean waitForComplete) {
    checkIfCalledOnValidThread();
    //AndLog.ShowLog(TAG, "Disonnect WebSocket. State: " + state);
        ...
        synchronized (closeEventLock) {
          while (!closeEvent) {
            try {
              ...
              //AndLog.ShowLog(TAG, "Wait error: " + e.toString());
            }
          }
        }
      }
    }
    //AndLog.ShowLog(TAG, "Disonnecting WebSocket done.");
  }

  private void reportError(final String errorMessage) {
   // AndLog.ShowLog(TAG, errorMessage);
  ...
  private void sendWSSMessage(final String method, final String message) {
   ...
   // AndLog.ShowLog(TAG, "WS " + method + " : " + postUrl + " : " + message);
  ...
  private class WebSocketObserver implements WebSocket.WebSocketConnectionObserver {
    @Override
    public void onOpen() {
      //AndLog.ShowLog(TAG, "WebSocket connection opened to: " + wsServerUrl);
      executor.execute(new Runnable() {
        @Override
        public void run() {
          state = WebSocketConnectionState.CONNECTED;
    ...
    public void onClose(WebSocketCloseNotification code, String reason) {
      /*AndLog.ShowLog(TAG, "WebSocket connection closed. Code: " + code
    ...
    public void onTextMessage(String payload) {
      //AndLog.ShowLog(TAG, "WSS->C: " + payload);
