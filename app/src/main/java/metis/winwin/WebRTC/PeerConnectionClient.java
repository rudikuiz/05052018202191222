...
public class PeerConnectionClient {
  ...
  public void createPeerConnection(
    ...
    if (peerConnectionParameters == null) {
      //AndLog.ShowLog(TAG, "Creating peer connection without initializing factory.");
      return;
    }
  ...
  private void createPeerConnectionFactoryInternal(Context context) {
    ...
    /*AndLog.ShowLog(TAG, "Create peer connection factory. Use video: " +
    ...
    //AndLog.ShowLog(TAG, "Pereferred video codec: " + preferredVideoCodec);
    ...
    if (!peerConnectionParameters.useOpenSLES) {
      //AndLog.ShowLog(TAG, "Disable OpenSL ES audio even if device supports it");
      WebRtcAudioManager.setBlacklistDeviceForOpenSLESUsage(true /* enable */);
    } else {
      //AndLog.ShowLog(TAG, "Allow OpenSL ES audio if device supports it");
    ...
    if (options != null) {
      //AndLog.ShowLog(TAG, "Factory networkIgnoreMask option: " + options.networkIgnoreMask);
    }
    factory = new PeerConnectionFactory(options);
    //AndLog.ShowLog(TAG, "Peer connection factory created.");
  }
  ...
  private void createMediaConstraintsInternal() {
    ...
    if (peerConnectionParameters.noAudioProcessing) {
      //AndLog.ShowLog(TAG, "Disabling audio processing");
  ...
  private void createPeerConnectionInternal(EglBase.Context renderEGLContext) {
    if (factory == null || isError) {
      //AndLog.ShowLog(TAG, "Peerconnection factory is not created");
      return;
    }
    //AndLog.ShowLog(TAG, "Create peer connection.");

    //AndLog.ShowLog(TAG, "PCConstraints: " + pcConstraints.toString());
    if (videoConstraints != null) {
      //AndLog.ShowLog(TAG, "VideoConstraints: " + videoConstraints.toString());
    }
    queuedRemoteCandidates = new LinkedList<IceCandidate>();

    if (videoCallEnabled) {
      //AndLog.ShowLog(TAG, "EGLContext: " + renderEGLContext);
    ...
    if (videoCallEnabled) {
      ...
      //AndLog.ShowLog(TAG, "Opening camera: " + cameraDeviceName);
    ...
    if (peerConnectionParameters.aecDump) {
      try {
        ...
        //AndLog.ShowLog(TAG, "Can not open aecdump file", e);
      }
    }

    //AndLog.ShowLog(TAG, "Peer connection created.");
  }
  ...
  private void closeInternal() {
    ...
    //AndLog.ShowLog(TAG, "Closing peer connection.");
    statsTimer.cancel();
    if (peerConnection != null) {
      peerConnection.dispose();
      peerConnection = null;
    }
    //AndLog.ShowLog(TAG, "Closing video source.");
    if (videoSource != null) {
      videoSource.dispose();
      videoSource = null;
    }
    //AndLog.ShowLog(TAG, "Closing peer connection factory.");
    if (factory != null) {
      factory.dispose();
      factory = null;
    }
    options = null;
    //AndLog.ShowLog(TAG, "Closing peer connection done.");
    ...
    for (KeyValuePair keyValuePair : videoConstraints.mandatory) {
      if (keyValuePair.getKey().equals("minWidth")) {
        try {
          ...
          //AndLog.ShowLog(TAG, "Can not parse video width from video constraints");
        }
      } else if (keyValuePair.getKey().equals("minHeight")) {
        try {
          minHeight = Integer.parseInt(keyValuePair.getValue());
        } catch (NumberFormatException e) {
          //AndLog.ShowLog(TAG, "Can not parse video height from video constraints");
        }
      }
    }
  ...
  private void getStats() {
    ...
    if (!success) {
      //AndLog.ShowLog(TAG, "getStats() returns false!");
    }
  }
  ...
  public void enableStatsEvents(boolean enable, int periodMs) {
    if (enable) {
      try {
        ...
        //AndLog.ShowLog(TAG, "Can not schedule statistics timer", e);
      }
    } else {
      statsTimer.cancel();
    }
  ...
  public void createOffer() {
    executor.execute(new Runnable() {
      ...
      public void run() {
        if (peerConnection != null && !isError) {
          //AndLog.ShowLog(TAG, "PC Create OFFER");
          isInitiator = true;
  ...
  public void createAnswer() {
    executor.execute(new Runnable() {
      ...
      public void run() {
        if (peerConnection != null && !isError) {
          //AndLog.ShowLog(TAG, "PC create ANSWER");
          isInitiator = false;
  ...
  public void setRemoteDescription(final SessionDescription sdp) {
    executor.execute(new Runnable() {
      ...
      public void run() {
        ...
        //AndLog.ShowLog(TAG, "Set remote SDP.");
    ...
    executor.execute(new Runnable() {
      ...
      public void run() {
        if (videoSource != null && !videoSourceStopped) {
          //AndLog.ShowLog(TAG, "Stop video source.");
          videoSource.stop();
  ...
  public void startVideoSource() {
    executor.execute(new Runnable() {
      ...
      public void run() {
        if (videoSource != null && videoSourceStopped) {
          //AndLog.ShowLog(TAG, "Restart video source.");
          videoSource.restart();
          videoSourceStopped = false;
        }
      }
  ...
  private void reportError(final String errorMessage) {
    //AndLog.ShowLog(TAG, "Peerconnection error: " + errorMessage);
    executor.execute(new Runnable() {
      @Override
      public void run() {
        if (!isError) {
  ...
  private static String setStartBitrate(String codec, boolean isVideoCodec,
    ...
    /*AndLog.ShowLog(TAG, "Found " +  codec + " rtpmap " + codecRtpMap
    ...
    for (int i = 0; i < lines.length; i++) {
      Matcher codecMatcher = codecPattern.matcher(lines[i]);
      if (codecMatcher.matches()) {
        //AndLog.ShowLog(TAG, "Found " +  codec + " " + lines[i]);
        if (isVideoCodec) {
        ...
        //AndLog.ShowLog(TAG, "Update remote SDP line: " + lines[i]);
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (!sdpFormatUpdated && i == rtpmapLineIndex) {
        ...
        //AndLog.ShowLog(TAG, "Add remote SDP line: " + bitrateSet);
  ...
  private static String preferCodec(
    ...
    /*AndLog.ShowLog(TAG, "Found " +  codec + " rtpmap " + codecRtpMap + ", prefer at "
        + lines[mLineIndex]);*/
    ...
    if (origMLineParts.length > 3) {
      ...
      //AndLog.ShowLog(TAG, "Change media description: " + lines[mLineIndex]);
    } else {
      //AndLog.ShowLog(TAG, "Wrong SDP media description format: " + lines[mLineIndex]);
    }
    StringBuilder newSdpDescription = new StringBuilder();
  ...
  private void drainCandidates() {
    if (queuedRemoteCandidates != null) {
      //AndLog.ShowLog(TAG, "Add " + queuedRemoteCandidates.size() + " remote candidates");
  ...
  private void switchCameraInternal() {
    if (!videoCallEnabled || numberOfCameras < 2 || isError || videoCapturer == null) {
      /*AndLog.ShowLog(TAG, "Failed to switch camera. Video: " + videoCallEnabled + ". Error : "
          + isError + ". Number of cameras: " + numberOfCameras);*/
      return;  // No video is sent or only one camera is available or error happened.
    }
    //AndLog.ShowLog(TAG, "Switch camera");
  ...
  private void changeCaptureFormatInternal(int width, int height, int framerate) {
    if (!videoCallEnabled || isError || videoCapturer == null) {
      /*AndLog.ShowLog(TAG, "Failed to change capture format. Video: " + videoCallEnabled + ". Error : "
          + isError);*/
      return;
    }
  ...
  private class PCObserver implements PeerConnection.Observer {
    ...
    public void onSignalingChange(
      ...
      //AndLog.ShowLog(TAG, "SignalingState: " + newState);
    }
    ...
    public void onIceConnectionChange(
      ...
      executor.execute(new Runnable() {
        @Override
        public void run() {
          //AndLog.ShowLog(TAG, "IceConnectionState: " + newState);
    ...
    public void onIceGatheringChange(
      ...
      //AndLog.ShowLog(TAG, "IceGatheringState: " + newState);
    }

    @Override
    public void onIceConnectionReceivingChange(boolean receiving) {
      //AndLog.ShowLog(TAG, "IceConnectionReceiving changed to " + receiving);
    }
  ...
  private class SDPObserver implements SdpObserver {
    ...
    public void onCreateSuccess(final SessionDescription origSdp) {
      ...
      executor.execute(new Runnable() {
        ...
        public void run() {
          if (peerConnection != null && !isError) {
            //AndLog.ShowLog(TAG, "Set local SDP from " + sdp.type);
        ...
        public void run() {
          ...
          if (isInitiator) {
            ...
            if (peerConnection.getRemoteDescription() == null) {
              ...
              //AndLog.ShowLog(TAG, "Local SDP set succesfully");
              events.onLocalDescription(localSdp);
            } else {
              // We've just set remote description, so drain remote
              // and send local ICE candidates.
              //AndLog.ShowLog(TAG, "Remote SDP set succesfully");
              drainCandidates();
            }
          } else {
